    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            fileInputStream.close(); // Закрываем поток / Close the stream
                ...
                FileNotFoundException exception) { // Обработка исключения, если файл не найден / Handle exception if file is not found
            System.out.println("File not found");
            exception.printStackTrace();
        } catch (
                IOException ioException) { // Обработка исключения при ошибке чтения / Handle exception if there is a reading error
